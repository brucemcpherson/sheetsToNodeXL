...
function Fiddler(sheet) {
  ...
  var _values,
    ...
    _formulas,
    _custom = null,
    _respectFilter = false,
    _respectHidden = false,
    _defaultFlat = {
      flatten: true,
      objectSeparator: ".",
      itemSeparator: ",",
  ...
  */
  ...
  /**
   * are we respecting filters
   */
  self.respectFilter = () => _respectFilter
  self.setRespectFilter = (value) => {
    _respectFilter = value
    return self
  }
  self.respectHidden = () => _respectHidden
  self.setRespectHidden = (value) => {
    _respectHidden = value
    return self
  }

  /**
   * get hidden row positions 
   * these start at 0
   * ie. the data starts at 0 regardless of whether there are row headers
   * so row 0 always points to the getData()[0]
   */
  self.applyRespect = (values) => {
    if (!self.respectFilter() && !self.respectHidden()) return values

    const dataStartsAt = self.hasHeaders() ? 2 : 1
    const sheet = self.getSheet()

    return values.filter((f, i) => !(
      (self.respectFilter() && sheet.isRowHiddenByFilter(i + dataStartsAt)) ||
      (self.respectHidden() && sheet.isRowHiddenByUser(i + dataStartsAt)))
    )
  }
  ...
  */
  ...
  self.populate = function (sheet) {
    ...
    // apply filters
    const values = self.applyRespect(range.getValues())
